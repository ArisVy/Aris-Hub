...
AddNotify({Title="ARIS HUB V1.0 - ENGLISH",Description="UPDATE: Prediction Fix (No Skill Lock)!",Duration=8})
...
_G.SharedAimPos = nil -- THÊM BIẾN TOÀN CỤC ĐỂ CHỨA TỌA ĐỘ PREDICTION
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.Config.SilentAim then
        ...
        -- TÍNH TOÁN PREDICTION VÀ LƯU VÀO BIẾN GLOBAL ĐỂ DÙNG BÊN TRONG HOOK
        if SilentAimTarget then
            local aimPos = SilentAimTarget.Position
            if _G.Config.Prediction_Enabled and SilentAimTarget:IsA("BasePart") then
                local vel = SilentAimTarget.AssemblyLinearVelocity
                local ping = 0
                pcall(function() ping = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue() / 1000 end)
                local safeVel = Vector3.new(vel.X, 0, vel.Z)
                aimPos = aimPos + (safeVel * (_G.Config.Prediction + ping))
            end
            _G.SharedAimPos = aimPos
        else
            _G.SharedAimPos = nil
        end
        ...
        if _G.Config.SilentAim and SilentAimTarget and self == workspace and not checkcaller() then
            if method == "Raycast" or string.find(method, "Ray") then
                ...
                if origin and typeof(origin) == "Vector3" then
                    -- LẤY TỌA ĐỘ PREDICTION TỪ BIẾN TOÀN CỤC (FIX LAG)
                    local aimPos = _G.SharedAimPos or SilentAimTarget.Position
