...
game:GetService("StarterGui"):SetCore("SendNotification",{
    ...
    Text="Nạp thành công! ESP Box bo góc mạnh + Viền Xanh/Đen/Đỏ/Hồng.",
    Duration=8
})
...
local function ApplyToggleGradient(parent, isOn)
    ...
    if txt then
        ...
        if isOn then
            ...
            if txtGrad then 
                txtGrad.Enabled = true 
                txtGrad:SetAttribute("CustomOnColor", false) 
            end
            txt.TextColor3 = Color3.fromRGB(255, 255, 255) 
        end
    end
    ...
    task.spawn(function()
        while _G.Config.TP_NPC or _G.Config.TP_Player do
            ...
            if myRoot then
                ...
                -- BƯỚC 1: KIỂM TRA MỤC TIÊU HIỆN TẠI CÒN HỢP LỆ KHÔNG (CƠ CHẾ KEO DÍNH)
                ...
                if currentTarget and currentTarget.Parent then
                    ...
                    if hum and hum.Health > 0 then
                        if _G.Config.TP_Player then
                            ...
                            if p and not _G.Config.BlacklistedPlayers[p.Name] and not TempSkipPlayer[currentTarget.Parent] then
                                targetValid = true
                            end
                        elseif _G.Config.TP_NPC then
                            if not _G.Config.BlacklistedNPCs[currentTarget.Parent.Name] and not TempSkipNPC[currentTarget.Parent] then
                                targetValid = true
                            end
                        end
                    end
                ...
                -- BƯỚC 2: NẾU MỤC TIÊU HIỆN TẠI ĐÃ CHẾT/BỊ SKIP -> TÌM MỤC TIÊU GẦN NHẤT MỚI
                    ...
                    elseif _G.Config.TP_Player then
                        for _, p in ipairs(Players:GetPlayers()) do
                            if p ~= LocalPlayer and p.Character then
                                ...
                                local isBL = _G.Config.BlacklistedPlayers[p.Name]
                                local isSkipped = TempSkipPlayer[p.Character]
                                
                                if not (_G.Config.TeamCheck and isSameTeam) and not isBL and not isSkipped then
                ...
                -- BƯỚC 3: BAY TỚI MỤC TIÊU ĐÃ CHỐT
...
AddButton("TP Player", "⏭️ BỎ QUA PLAYER NÀY (SKIP)", function()
    if currentTarget and currentTarget.Parent then
        TempSkipPlayer[currentTarget.Parent] = true
        local pName = currentTarget.Parent.Name
        currentTarget = nil
        if currentTween then currentTween:Cancel() end
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title="SKIP PLAYER", Text="Đã tạm thời bỏ qua " .. pName .. "!", Duration=3
        })
    end
end)

local PlayerBLContainer = Instance.new("Frame", ContentFrames["TP Player"].Frame)
PlayerBLContainer.Size = UDim2.new(1, -16, 0, 0)
PlayerBLContainer.BackgroundTransparency = 1
local pblLayout = Instance.new("UIListLayout", PlayerBLContainer)
pblLayout.Padding = UDim.new(0, 8)
pblLayout.SortOrder = Enum.SortOrder.LayoutOrder
pblLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    PlayerBLContainer.Size = UDim2.new(1, -16, 0, pblLayout.AbsoluteContentSize.Y)
end)

AddButton("TP Player", "🔄 LÀM MỚI PLAYER (10KM)", function()
    for _, child in ipairs(PlayerBLContainer:GetChildren()) do
        if child:IsA("TextButton") then child:Destroy() end
    end
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        -- Quét phạm vi 10,000 studs (10km)
        if root and (root.Position - myRoot.Position).Magnitude <= 10000 then
            count = count + 1
            local btn = Instance.new("TextButton", PlayerBLContainer)
            btn.Size = UDim2.new(1, 0, 0, 36)
            btn.Text = ""
            btn.BackgroundColor3 = Color3.new(1,1,1)
            Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 20)
            
            local isBlacklisted = _G.Config.BlacklistedPlayers[p.Name] or false
            ApplyToggleGradient(btn, isBlacklisted)
            CreateBorder(btn)
            local txt = CreateButtonText(btn, "🚫 BL: " .. p.Name, Enum.Font.GothamBold, 12)
            if isBlacklisted then txt.TextColor3 = Color3.fromRGB(0,0,0) end
            ApplyButtonAnimation(btn)
            
            btn.MouseButton1Click:Connect(function()
                _G.Config.BlacklistedPlayers[p.Name] = not _G.Config.BlacklistedPlayers[p.Name]
                local state = _G.Config.BlacklistedPlayers[p.Name]
                ApplyToggleGradient(btn, state)
                txt.TextColor3 = state and Color3.fromRGB(0,0,0) or Color3.new(1,1,1)
            end)
        end
    ...
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title="TÌM KIẾM PLAYER", Text="Đã quét thấy " .. count .. " người chơi quanh đây!", Duration=3
    })
end)
    ...
    for _,p in Players:GetPlayers()do
            ...
            if valid then
                ...
                if hrp.Size ~= targetSize then 
                    if not hrp:GetAttribute("ArisOrigSizeX") then
                        ...
                        hrp:SetAttribute("ArisOrigMassless", hrp.Massless)
                    end
                    hrp.Size = targetSize
        ...
        if _G.Config.ESP_Name_P or _G.Config.ESP_Health_P or _G.Config.ESP_Distance_P or _G.Config.ESP_Box_P then
            if not ESP_Store[p.Name] then
                ...
                -- THÊM BO GÓC (CORNER RADIUS) MẠNH LÊN
                ...
                -- CHÈN ĐEN CHUYỂN ĐỎ VÀO GIỮA LỚP GRADIENT XANH HỒNG
                grad.Color = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 230, 255)),   -- Xanh
                    ColorSequenceKeypoint.new(0.35, Color3.fromRGB(0, 0, 0)),    -- Đen
                    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 0, 0)),   -- Đỏ (Giữa)
                    ColorSequenceKeypoint.new(0.65, Color3.fromRGB(0, 0, 0)),    -- Đen
                    ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 50, 200))   -- Hồng
                })
                
                local cDot = Instance.new("Frame", boxBill); cDot.Size = UDim2.new(0,4,0,4); cDot.Position = UDim2.new(0.5,-2,0.5,-2); cDot.BackgroundColor3 = Color3.fromRGB(0,230,255); cDot.BorderSizePixel=0;
                Instance.new("UICorner", cDot).CornerRadius = UDim.new(1,0)
                local hLine = Instance.new("Frame", boxBill); hLine.Size = UDim2.new(0.5,0,0,1); hLine.Position = UDim2.new(0.25,0,0.5,0); hLine.BackgroundColor3 = Color3.new(1,1,1); hLine.BackgroundTransparency = 0.5;
                hLine.BorderSizePixel=0
                local vLine = Instance.new("Frame", boxBill); vLine.Size = UDim2.new(0,1,0.5,0); vLine.Position = UDim2.new(0.5,0,0.25,0); vLine.BackgroundColor3 = Color3.new(1,1,1); vLine.BackgroundTransparency = 0.5;
            ...
            s.BoxBill.Enabled=_G.Config.ESP_Box_P; if s.Grad then s.Grad.Rotation = (tick() * 150) % 360 end
