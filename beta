...
game:GetService("StarterGui"):SetCore("SendNotification",{
    ...
    Text="Cập nhật: Text gọn gàng, Hiệu ứng nút thông minh!",
    Duration=8
})
...
_G.Config={
    ...
    Desync_ShowFloat = false,  -- MẶC ĐỊNH TẮT
    Desync_FloatX = 70,        -- TỌA ĐỘ X: 70
    Desync_FloatY = 20,        -- TỌA ĐỘ Y: 20
    Desync_Mode = "Normal"
}
...
-- Hàm xử lý đổi màu chữ khi bật/tắt (ĐEN KHI BẬT, MÀU KHI TẮT)
local function ApplyToggleGradient(parent, isOn)
    ...
    if txt then
        ...
        if isOn then
            ...
            txt.TextColor3 = Color3.fromRGB(0, 0, 0) -- Chữ đen khi bật
        else
            if txtGrad then txtGrad.Enabled = true end
            txt.TextColor3 = Color3.fromRGB(255, 255, 255) -- Chữ gradient khi tắt
        end
    end
end
...
for i,tab in ipairs(Tabs)do
    ...
    CreateBorder(btn) 
    CreateButtonText(btn, tab, Enum.Font.GothamBold, 12)
