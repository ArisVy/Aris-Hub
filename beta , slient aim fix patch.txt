...
AddNotify({Title="ARIS HUB V1.0 - UPDATED",Description="Safe Mode Fly 300, Silent Aim Prediction 0.127s",Duration=8})
    ...
    OldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if _G.Config.SilentAim and SilentAimTarget and SAPredictedPos and not checkcaller() then
            ...
            if method == "FireServer" then
                ...
                elseif rName == "LeftClickRemote" then
                    if not _G.Config.SilentAim_M1 or IsHoldingFruit() then 
                        return OldNamecall(self, ...) 
                    end
                    local char = LocalPlayer.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    if hrp and typeof(args[1]) == "Vector3" then
                        args[1] = (SAPredictedPos - hrp.Position).Unit
                    end
                    return OldNamecall(self, unpack(args))
                end
            end
...
AddGridToggle(nullGrid, "M1 AIMBOT", "SilentAim_M1")
