...
RunService.Heartbeat:Connect(function()
    ...
    for _,p in Players:GetPlayers()do
        ...
        if enableHitbox then
            ...
            if valid then
                if not hrp:GetAttribute("ArisOrigSizeX") then
                    ...
                    hrp:SetAttribute("ArisOrigSizeY", hrp.Size.Y); hrp:SetAttribute("ArisOrigSizeZ", hrp.Size.Z)
                    hrp:SetAttribute("ArisOrigTrans", hrp.Transparency);
                    hrp:SetAttribute("ArisOrigCollide", hrp.CanCollide); hrp:SetAttribute("ArisOrigMassless", hrp.Massless)
                end
        ...
        if enableHitbox and _G.Config.Hitbox_Box then
            if not hbBox then 
                hbBox = Instance.new("SelectionBox", hrp);
                hbBox.Name = "ArisHitboxBox"; hbBox.Adornee = hrp 
            end
        ...
        if _G.Config.ESP_Name_P or _G.Config.ESP_Health_P or _G.Config.ESP_Distance_P or _G.Config.ESP_Box_P then
            if not ESP_Store[p.Name] then
                ...
                boxBill.Size = UDim2.new(4.2,0,5.8,0); boxBill.AlwaysOnTop = true
                
                local outF = Instance.new("Frame", boxBill);
                outF.Size = UDim2.new(1,0,1,0); outF.BackgroundTransparency = 1
                ...
                local cDot = Instance.new("Frame", boxBill);
                cDot.Size = UDim2.new(0,4,0,4); cDot.Position = UDim2.new(0.5,-2,0.5,-2); cDot.BackgroundColor3 = Color3.fromRGB(0,230,255); cDot.BorderSizePixel=0;
                Instance.new("UICorner", cDot).CornerRadius = UDim.new(1,0)
                local hLine = Instance.new("Frame", boxBill);
                hLine.Size = UDim2.new(0.5,0,0,1); hLine.Position = UDim2.new(0.25,0,0.5,0); hLine.BackgroundColor3 = Color3.new(1,1,1); hLine.BackgroundTransparency = 0.5;
                hLine.BorderSizePixel=0
                local vLine = Instance.new("Frame", boxBill);
                vLine.Size = UDim2.new(0,1,0.5,0); vLine.Position = UDim2.new(0.5,0,0.25,0); vLine.BackgroundColor3 = Color3.new(1,1,1); vLine.BackgroundTransparency = 0.5;
                vLine.BorderSizePixel=0
                
                local textB = Instance.new("BillboardGui", ScreenGui);
                textB.Size = UDim2.new(0,200,0,60); textB.StudsOffset = Vector3.new(0,3.5,0); textB.AlwaysOnTop = true
        ...
        if hum and hrp and hum.Health > 0 then
            ...
            if _G.Config.Hitbox_NPC and _G.Config.Hitbox_Box_NPC then
                if not hbBoxNPC then 
                    ...
                    hbBoxNPC.Name = "ArisHitboxBoxNPC"; hbBoxNPC.Adornee = hrp 
                end
            ...
            if _G.Config.ESP_NPC_Name or _G.Config.ESP_NPC_Box then
                if not NPC_Store[obj] then
                    local textB = Instance.new("BillboardGui", ScreenGui); textB.Size = UDim2.new(0, 200, 0, 60); textB.StudsOffset = Vector3.new(0, 3.5, 0);
                    ...
                    local boxBill = Instance.new("BillboardGui", ScreenGui);
                    boxBill.Size = UDim2.new(4,0,5.5,0); boxBill.AlwaysOnTop = true
                    local outF = Instance.new("Frame", boxBill);
                    outF.Size = UDim2.new(1,0,1,0); outF.BackgroundTransparency = 1
                    Instance.new("UICorner", outF).CornerRadius = UDim.new(0, 6)
                    local inS = Instance.new("UIStroke", outF);
                    inS.Thickness = 1.5; inS.Color = Color3.new(1,1,1)
