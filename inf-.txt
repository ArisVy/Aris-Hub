...
AddNotify({Title="ARIS HUB V1.0",Description="Update: M1 Aimbot Removed, Fly Auto Force 300, Pull Down 1k!",Duration=8})
...
-- [[ START OF NEW LOGIC FROM STREEHUB ]] --

local function getInfinity_Ability(ability, isEnabled)
    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
    
    if ability == "Soru" and isEnabled then
        for _, v in next, getgc() do
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Soru") then
                if typeof(v) == "function" and getfenv(v).script == LocalPlayer.Character.Soru then
                    for _, upval in next, getupvalues(v) do
                        if typeof(upval) == "table" then
                            task.spawn(function()
                                repeat
                                    task.wait(0.1)
                                    if typeof(upval) == "table" and upval.LastUse then
                                        upval.LastUse = 0
                                    end
                                until not _G.Config.InfSoru or LocalPlayer.Character.Humanoid.Health <= 0
                            end)
                        end
                    end
                end
    ...
    elseif ability == "Energy" and isEnabled then
        if LocalPlayer.Character:FindFirstChild("Energy") then
            local maxEnergy = LocalPlayer.Character.Energy.Value
            task.spawn(function()
                while _G.Config.InfEnergy and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Energy") do
                    LocalPlayer.Character.Energy.Value = maxEnergy
                    task.wait(0.1)
                end
            end)
        end
...
-- Function to handle continuous Mink V3 ability
local function handleInfMink()
    task.spawn(function()
        while _G.Config.InfMink do
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") and LocalPlayer.Character.Humanoid.Health > 0 then
                 -- This will constantly fire the V3 skill event for Mink if enabled
                pcall(function()
                     ReplicatedStorage.Remotes.CommE:FireServer("ActivateRaceSkill")
                end)
            end
            task.wait(0.5) -- Small delay to prevent spamming the remote too hard
        end
    end)
end
...
-- [[ ADDING NEW TOGGLES TO MISC GRID ]] --
AddGridToggle(miscGrid, "INF SORU", "InfSoru", function(val) getInfinity_Ability("Soru", val) end)
AddGridToggle(miscGrid, "INF ENERGY", "InfEnergy", function(val) getInfinity_Ability("Energy", val) end)
AddGridToggle(miscGrid, "INF MINK V3", "InfMink", function(val) if val then handleInfMink() end end)
...
local blackRedColors = ColorSequence.new({ ColorSequenceKeypoint.new(0, 0, 0), ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 0, 0)) })
...
if _G.M1BF then
    ...
    task.spawn(function()
        while _G.M1BF and task.wait() do
            ...
            if tick() - lastAttack1 < (_G.Config.FastM1_Delay or 0) then continue end 
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if myRoot then
        ...
        if hum and hum.Health > 0 then
            ...
            if _G.Config.SafeMode then
                if hpPct <= 0.35 and not _G.IsFleeing then
                    _G.IsFleeing = true
                    if currentTween then currentTween:Cancel() end
                    if not noclipConnection then toggleNoclip(true) end
                elseif hpPct > 0.65 and _G.IsFleeing then
                    _G.IsFleeing = false
                    local bv = myRoot:FindFirstChild("ArisFlightForce")
                    if bv then bv:Destroy() end
        ...
        if _G.Config.AutoDrop and not _G.Config.SafeMode then
            if myRoot.Position.Y > 40000 and not _G.IsFleeing and not _G.IsForcedDropping then
                _G.IsForcedDropping = true
                if currentTween then currentTween:Cancel() end
                if not noclipConnection then toggleNoclip(true) end
            end
        else
            if _G.IsForcedDropping and not (_G.IsForcedDropping and myRoot.Position.Y <= 150) then
                _G.IsForcedDropping = false
                local bv = myRoot:FindFirstChild("ArisFlightForce")
                if bv then bv:Destroy() end
                if noclipConnection and not isFarming then toggleNoclip(false) end
        ...
        -- Vật lý bay lên/rớt xuống
        if _G.IsFleeing then
            local flightBv = myRoot:FindFirstChild("ArisFlightForce")
            if not flightBv then
                flightBv = Instance.new("BodyVelocity")
                flightBv.Name = "ArisFlightForce"
                flightBv.MaxForce = Vector3.new(9e9, 9e9, 9e9)
                flightBv.Parent = myRoot
            end
            flightBv.Velocity = Vector3.new(0, 300, 0)
        elseif _G.IsForcedDropping then
            local flightBv = myRoot:FindFirstChild("ArisFlightForce")
            if not flightBv then
                flightBv = Instance.new("BodyVelocity")
                flightBv.Name = "ArisFlightForce"
            ...
            flightBv.Velocity = Vector3.new(0, -1000, 0)
        end
    end
