...
Text="Đã thêm mục TP NPC và TP Player!",
Duration=8
})
...
ToggleBtn.Visible = _G.Config.Show_ToggleBtn
...
for _,p in Players:GetPlayers()do
    ...
    if _G.Config.ESP_Name_P or _G.Config.ESP_Health_P or _G.Config.ESP_Distance_P or _G.Config.ESP_Box_P then
        if not ESP_Store[p.Name] then
            ...
            outS.Color = Color3.new(0,0,0); outS.Thickness = 3
            
            local inF = Instance.new("Frame", boxBill);
            inF.Size = UDim2.new(1,0,1,0); inF.BackgroundTransparency = 1
            Instance.new("UICorner", inF).CornerRadius = UDim.new(0, 6)
            
            local inS = Instance.new("UIStroke", inF);
            inS.Thickness = 1.2
    ...
    if hum and hrp and hum.Health > 0 then
        ...
        if _G.Config.ESP_NPC_Name then
            if not NPC_Store[obj] then
                local textB = Instance.new("BillboardGui", ScreenGui); 
                textB.Size = UDim2.new(0, 200, 0, 60); textB.StudsOffset = Vector3.new(0, 3.5, 0);
            ...
            if NPC_Store[obj] then NPC_Store[obj].Bill:Destroy();
            NPC_Store[obj] = nil end
        end
    else 
        CleanupNPC(obj) 
    end
