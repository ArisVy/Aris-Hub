...
ContentFrames["Chống Sét"].Frame.Visible=true
...
function AddToggle(tab,name,key,cb)
    ...
    ApplyToggleGradient(btn, _G.Config[key]) CreateBorder(btn) local btnTxt = CreateButtonText(btn, name..": "..(_G.Config[key]and"BẬT"or"TẮT"), Enum.Font.GothamBold, 14) ApplyButtonAnimation(btn)
    ToggleButtons[key] = {Btn = btn, Txt = btnTxt, Name = name}
    btn.MouseButton1Click:Connect(function() _G.Config[key] = not _G.Config[key] btnTxt.Text = name..": "..(_G.Config[key]and"BẬT"or"TẮT") ApplyToggleGradient(btn, _G.Config[key]) if cb then cb(_G.Config[key])end end)
end

-- =================================================================
-- LOGIC CHỐNG SÉT TÍCH HỢP
-- =================================================================
local sweepConnection = nil
local loopConnection = nil
local lastAlertTime = 0

local StatusLabel = AddLabel("Chống Sét", "⏱️ Trạng thái: Chưa kích hoạt")
...
local function triggerEarlyLightningAlert(customMsg)
    ...
    StatusLabel.Text = "⚠️ BÁO ĐỘNG SỚM: SÉT SẮP ĐÁNH TRONG 1-2S TỚI!"
    
    AddNotify({
        Title = "⚡ BÁO ĐỘNG ĐỎ!",
        Description = customMsg or "Thu hoạch ngay! Tín hiệu sét đã xuất hiện trước 1-2 giây!",
        Duration = 3,
    })

    task.delay(3, function()
        if _G.Config.LightningAlert then
            StatusLabel.Text = "⏱️ Trạng thái: Đang canh chừng vòng tiếp theo..."
        end
    end)
end
...
local function startMonitoring()
    ...
    sweepConnection = Workspace.DescendantAdded:Connect(function(child)
        ...
        if isEarlyLightningSignal(child) and isNearPlayer(child) then
            triggerEarlyLightningAlert("Sét sắp đánh xuống vùng cây gần bạn! Thu hoạch ngay!")
        end
    end)
...
AddToggle("Chống Sét", "CẢNH BÁO SÉT", "LightningAlert", function(Value)
    if Value then
        StatusLabel.Text = "⏱️ Trạng thái: Đang canh chừng vòng tiếp theo..."
        startMonitoring()
        AddNotify({Title="BẬT CHỐNG SÉT", Description="Đã kích hoạt hệ thống cảnh báo sớm!", Duration=2})
    else
        StatusLabel.Text = "⏱️ Trạng thái: Đã tắt canh chừng"
        ...
        AddNotify({Title="TẮT CHỐNG SÉT", Description="Đã dừng hệ thống quét!", Duration=2})
    end
end)
