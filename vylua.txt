...
game:GetService("StarterGui"):SetCore("SendNotification",{
    ...
    Text="Nạp thành công! Walkspeed liên tục & 3D Hitbox Gradient.",
    Duration=8
})
...
-- HÀM LẤY MÀU GRADIENT 3D HITBOX
local function GetHitboxGradientColor(shift)
    local t = shift % 1
    if t < 0.35 then
        return Color3.fromRGB(0, 230, 255):Lerp(Color3.fromRGB(0, 0, 0), t / 0.35)
    elseif t < 0.5 then
        return Color3.fromRGB(0, 0, 0):Lerp(Color3.fromRGB(255, 0, 0), (t - 0.35) / 0.15)
    elseif t < 0.65 then
        return Color3.fromRGB(255, 0, 0):Lerp(Color3.fromRGB(0, 0, 0), (t - 0.5) / 0.15)
    else
        return Color3.fromRGB(0, 0, 0):Lerp(Color3.fromRGB(255, 50, 200), (t - 0.65) / 0.35)
    end
end
...
-- [ĐÃ SẮP XẾP LẠI UI] LOW HP KS LÊN ĐẦU
AddToggle("Hitbox","LOW HP KS (<30%)","LowHP_KS")
...
-- [ĐÃ SẮP XẾP LẠI UI] WALKSPEED NẰM DƯỚI TEAM CHECK
...
-- [FIX FREEZE]: HÀM NÀY ĐÃ BỎ LỆNH ÉP SIZE ĐỂ FIX CHẾT VẬT LÝ
local function RestoreHRP(hrp)
    if hrp and hrp:GetAttribute("ArisOrigTrans") then
...
-- [VÒNG LẶP WALKSPEED LIÊN TỤC] - Chạy cực nhanh bằng task.spawn + Stepped:Wait()
task.spawn(function()
    while true do
...
RunService.Heartbeat:Connect(function()
    ...
    for _,p in Players:GetPlayers()do
        ...
        if enableHitbox then
            ...
            if valid then
                if not hrp:GetAttribute("ArisOrigTrans") then
        ...
        if enableHitbox and _G.Config.Hitbox_Box then
            ...
            -- NÂNG CẤP ESP 3D HITBOX: VIỀN DÀY GRADIENT + MÀN XANH HƠI TRONG
            hbBox.Color3 = GetHitboxGradientColor(tick() * 0.5)
            hbBox.LineThickness = 0.15
            hbBox.SurfaceColor3 = Color3.fromRGB(0, 150, 255)
    ...
    for obj in pairs(CachedNPCs) do
        ...
        if hum and hrp and hum.Health > 0 then
            ...
            if _G.Config.Hitbox_NPC and _G.Config.Hitbox_Box_NPC then
                ...
                hbBoxNPC.Color3 = GetHitboxGradientColor(tick() * 0.5)
                hbBoxNPC.LineThickness = 0.15
                hbBoxNPC.SurfaceColor3 = Color3.fromRGB(0, 150, 255)
