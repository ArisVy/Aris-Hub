...
game:GetService("StarterGui"):SetCore("SendNotification",{
    Title="ARIS HUB V1.0",
    Text="Đã cập nhật hệ thống Desync V2 & Tracking ổn định!",
    Duration=8
})
...
-- ===============================================================
-- HỆ THỐNG DESYNC V2 TỪ FILE DESYNC1
-- ===============================================================
local GENV = getgenv()
...
function AnimationLib.CreateBeam(startPos, endPos, color, duration)
	...
	task.delay(duration, function() beam:Destroy() attachment1:Destroy() attachment2:Destroy() part:Destroy() end)
	return beam
end
...
local function FastRespawnUserLogic(plr, isHide)
    ...
    task.spawn(function()
        ...
        if isHide then
            local vheight = math.random(5000, 9888) newHrp.CFrame = ogpos + Vector3.new(0, vheight, 0) workspace.CurrentCamera.CFrame = ogpos2 task.wait(0.5) newHrp.CFrame = ogpos
        else
...
local function SetNormal(state) GENV.DesyncNormal = state if not state then ToggleDesync(false) end end
local function SetFast(state) GENV.DesyncFast = state if not state then ToggleDesync(false) end end
local function SetFixV2_Logic(state) GENV.DesyncFix = state ToggleDesync(state) if state then for _, flagData in ipairs(NumericFlags) do pcall(function() setfflag(flagData[1], flagData[2]) end) end end end
...
local function CreateDesyncMarker(pos)
	...
	task.spawn(function()
		while desyncPart and desyncPart.Parent do
			local t1 = TweenService:Create(desyncPart, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {Size = Vector3.new(4.5, 4.5, 4.5)}) t1:Play() t1.Completed:Wait()
			if desyncPart then local t2 = TweenService:Create(desyncPart, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {Size = Vector3.new(4, 4, 4)}) t2:Play() t2.Completed:Wait() end
		end
	end)
...
local function DoHideNormal()
    ...
    ToggleDesync(true)
	AnimationLib.DesyncTeleportEffect(hrp.Position) UpdateDesyncMarker(originalCF)
	...
	task.wait(0.5)
	GENV.DesyncNormal = true hrp.CFrame = originalCF workspace.CurrentCamera.CFrame = originalCam
...
local function ActivateDesyncNormal()
	...
	if GENV.HideAuto then DoHideNormal() else ToggleDesync(true) GENV.DesyncNormal = true UpdateDesyncMarker(hrp.CFrame) AnimationLib.DesyncTeleportEffect(hrp.Position) end
end

local function DoFastDesync()
	local char = LocalPlayer.Character if not char then return end
	local hrp = char:FindFirstChild("HumanoidRootPart") if not hrp then return end
    local savedCFrame = hrp.CFrame UpdateDesyncMarker(savedCFrame) FastRespawnUserLogic(LocalPlayer, GENV.HideAuto) SetFast(true)
...
local function DoFixDesync(isHide)
    local char = LocalPlayer.Character if not char then return end
	local hrp = char:FindFirstChild("HumanoidRootPart") if not hrp then return end
	SetFixV2_Logic(true) UpdateDesyncMarker(hrp.CFrame)
	...
	SetFixV2_Logic(true) UpdateDesyncMarker(newHrp.CFrame)
end
...
SafeBtn.MouseButton1Click:Connect(function()
    ...
    if not _G.Config.SafeMode and _G.Config.FastDrop and hrp and hrp.Position.Y > 40000 then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", { Title="SAFE MODE", Text=_G.Config.SafeMode and "BẬT: Bay tức thời 100km khi HP <35%!" or "Đã TẮT!", Duration=3 })
    end
end)
...
local espGrid = Instance.new("Frame", espTab) espGrid.Size = UDim2.new(1, -16, 0, 0) espGrid.BackgroundTransparency = 1
local espLayout = Instance.new("UIGridLayout", espGrid) espLayout.CellSize = UDim2.new(0.48, 0, 0, 36) espLayout.CellPadding = UDim2.new(0.04, 0, 0, 8) espLayout.SortOrder = Enum.SortOrder.LayoutOrder
espLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() espGrid.Size = UDim2.new(1, -16, 0, espLayout.AbsoluteContentSize.Y) end)
local function AddGridToggle(parent, name, key, cb)
    local btn = Instance.new("TextButton", parent) btn.BackgroundColor3 = Color3.new(1,1,1) btn.Text = "" Instance.new("UICorner",btn).CornerRadius = UDim.new(0,20) ApplyToggleGradient(btn, _G.Config[key]) CreateBorder(btn) local btnTxt = CreateButtonText(btn, name..": "..(_G.Config[key] and "ON" or "OFF"), Enum.Font.GothamBold, 11) ApplyButtonAnimation(btn) ToggleButtons[key] = {Btn = btn, Txt = btnTxt, Name = name}
    btn.MouseButton1Click:Connect(function() _G.Config[key] = not _G.Config[key] btnTxt.Text = name..": "..(_G.Config[key] and "ON" or "OFF") ApplyToggleGradient(btn, _G.Config[key]) if cb then cb(_G.Config[key]) end end)
end
AddGridToggle(espGrid, "NAME", "ESP_Name_P") AddGridToggle(espGrid, "HEALTH", "ESP_Health_P") AddGridToggle(espGrid, "DISTANCE", "ESP_Distance_P") AddGridToggle(espGrid, "BOX 2D", "ESP_Box_P") AddGridToggle(espGrid, "CHAMS", "ESP_Chams_P") AddGridToggle(espGrid, "PVP ESP", "ESP_PVP") 

AddToggle("Hitbox","HITBOX PLAYER","Hitbox_P") AddAdjust("Hitbox","HITBOX SIZE","HitboxSize",10) AddToggle("Hitbox","SHOW HITBOX BOX 3D","Hitbox_Box") AddToggle("Hitbox","ESP 2D THEO HITBOX","ESP_2D_Hitbox") AddToggle("Hitbox","HITBOX WALL CHECK","Hitbox_WallCheck")

local dualRowMisc = Instance.new("Frame", ContentFrames["Misc"].Frame) dualRowMisc.Size = UDim2.new(1, -16, 0, 36) dualRowMisc.BackgroundTransparency = 1
local tcBtn = Instance.new("TextButton", dualRowMisc) tcBtn.Size = UDim2.new(0.48, 0, 1, 0) tcBtn.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", tcBtn).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(tcBtn, _G.Config.TeamCheck) CreateBorder(tcBtn) local tcTxt = CreateButtonText(tcBtn, "TEAM CHECK: ON", Enum.Font.GothamBold, 11) ApplyButtonAnimation(tcBtn)
tcBtn.MouseButton1Click:Connect(function() _G.Config.TeamCheck = not _G.Config.TeamCheck tcTxt.Text = "TEAM CHECK: " .. (_G.Config.TeamCheck and "ON" or "OFF") ApplyToggleGradient(tcBtn, _G.Config.TeamCheck) end)
local pvpCBtn = Instance.new("TextButton", dualRowMisc) pvpCBtn.Size = UDim2.new(0.48, 0, 1, 0) pvpCBtn.Position = UDim2.new(0.52, 0, 0, 0) pvpCBtn.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", pvpCBtn).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(pvpCBtn, _G.Config.PVPCheck) CreateBorder(pvpCBtn) local pvpCTxt = CreateButtonText(pvpCBtn, "PVP CHECK: ON", Enum.Font.GothamBold, 11) ApplyButtonAnimation(pvpCBtn)
pvpCBtn.MouseButton1Click:Connect(function() _G.Config.PVPCheck = not _G.Config.PVPCheck pvpCTxt.Text = "PVP CHECK: " .. (_G.Config.PVPCheck and "ON" or "OFF") ApplyToggleGradient(pvpCBtn, _G.Config.PVPCheck) end)

AddToggle("Misc","LOW HP KS (<30%)","LowHP_KS") AddToggle("Misc","HIỆN FPS & PING","Show_Stats", function(val) StatsFrame.Visible = val end) AddToggle("Misc","FAST M1 (AUTO CLICK)","FastM1")
...
local WSSliderFill = Instance.new("Frame", WSSliderBg) WSSliderFill.Size = UDim2.new((_G.WalkSpeed - 16) / (250 - 16), 0, 1, 0) WSSliderFill.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", WSSliderFill).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(WSSliderFill, true)
local WSValLabel = Instance.new("TextLabel", WSSliderBg) WSValLabel.Size = UDim2.new(1, 0, 1, 0) WSValLabel.BackgroundTransparency = 1 WSValLabel.Text = "Speed: " .. _G.WalkSpeed WSValLabel.Font = Enum.Font.GothamBold WSValLabel.TextSize = 12 CreateTextGradient(WSValLabel)
...
local function UpdateWS(val) _G.WalkSpeed = math.clamp(val, 16, 250) local ratio = (_G.WalkSpeed - 16) / (250 - 16) WSSliderFill.Size = UDim2.new(ratio, 0, 1, 0) WSValLabel.Text = "Speed: " .. math.floor(_G.WalkSpeed) end
m10.MouseButton1Click:Connect(function() UpdateWS(_G.WalkSpeed - 10) end) m5.MouseButton1Click:Connect(function() UpdateWS(_G.WalkSpeed - 5) end) p5.MouseButton1Click:Connect(function() UpdateWS(_G.WalkSpeed + 5) end) p10.MouseButton1Click:Connect(function() UpdateWS(_G.WalkSpeed + 10) end)
WSToggle.MouseButton1Click:Connect(function() _G.WalkSpeedEnabled = not _G.WalkSpeedEnabled WSToggleTxt.Text = "WALKSPEED: " .. (_G.WalkSpeedEnabled and "ON" or "OFF") ApplyToggleGradient(WSToggle, _G.WalkSpeedEnabled) end)
local draggingWS = false WSSliderBg.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingWS = true end end)
...
AddToggle("NPC","HITBOX NPC","Hitbox_NPC") AddAdjust("NPC","HITBOX SIZE NPC","HitboxSize_NPC",10) AddToggle("NPC","SHOW HITBOX BOX 3D","Hitbox_Box_NPC") AddToggle("NPC","ESP NPC NAME","ESP_NPC_Name") AddToggle("NPC","ESP NPC BOX 2D","ESP_NPC_Box") AddToggle("NPC","ESP NPC CHAMS","ESP_NPC_Chams")
...
local function createModeBtn(text, posScale, modeStr)
    ...
    local function UpdateState() ApplyToggleGradient(btn, GENV.SelectedMode == modeStr) end UpdateState()
    btn.MouseButton1Click:Connect(function()
        ...
        GENV.SelectedMode = modeStr
        for _, child in ipairs(ModeFrame:GetChildren()) do if child:IsA("TextButton") then ApplyToggleGradient(child, child:GetAttribute("ModeStr") == GENV.SelectedMode) end end
...
floatBtn.MouseButton1Click:Connect(function()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p.UserId == adminUserId then isAdminInServer = true break end
    end
    if isAdminInServer and LocalPlayer.UserId ~= adminUserId then
        game:GetService("StarterGui"):SetCore("SendNotification", { Title="SYSTEM ALERT", Text="Desync creator is here, disabled.", Duration=5 }) return
    end
    ...
    if desyncState then 
        if GENV.SelectedMode == "Fix" then DoFixDesync(GENV.HideAuto) 
        elseif GENV.SelectedMode == "Fast" then DoFastDesync() 
...
Players.PlayerAdded:Connect(function(p)
    if p.UserId == adminUserId and LocalPlayer.UserId ~= adminUserId then
        if desyncState then desyncState = false RefreshFloatBtn() SetFast(false) SetNormal(false) SetFixV2_Logic(false) HideDesyncMarker() end
    end
end)

AddToggle("Desync", "GHOST MODE (👻)", "Desync_HideAuto", function() GENV.HideAuto = _G.Config.Desync_HideAuto if RefreshFloatBtn then RefreshFloatBtn() end end)
            ...
            if myRoot then
                    ...
                    if hum and hum.Health > 0 then
                        if _G.Config.TP_Player then
                            ...
                            if p and not TempSkipPlayer[p.Name] then
                                ...
                                if _G.Config.PVPCheck and not GetTrueStatus(p) then validToKeep = false end
                                if validToKeep then
                                    ...
                                    else keepCurrentTarget = true end
                                end
                            end
                    ...
                    if _G.Config.TP_NPC then
                        for npc, _ in pairs(CachedNPCs) do
                            if npc and npc.Parent then
                                ...
                                if _G.Config.SelectedTargetNPC and _G.Config.SelectedTargetNPC ~= "" and npc.Name ~= _G.Config.SelectedTargetNPC then continue end
                                local hum = npc:FindFirstChild("Humanoid") local root = npc:FindFirstChild("HumanoidRootPart") or npc:FindFirstChild("Hitbox") or npc:FindFirstChild("Torso")
                                if hum and root and hum.Health > 0 and root:IsA("BasePart") then
                    ...
                    elseif _G.Config.TP_Player then
                        for _, p in ipairs(Players:GetPlayers()) do
                            if p ~= LocalPlayer and p.Character and not TempSkipPlayer[p.Name] then
                                if _G.Config.SelectedTargetPlayer and _G.Config.SelectedTargetPlayer ~= "" and p.Name ~= _G.Config.SelectedTargetPlayer then continue end
                                if not (_G.Config.TeamCheck and LocalPlayer.Team and p.Team == LocalPlayer.Team) then
                                    if not _G.Config.PVPCheck or GetTrueStatus(p) then
                                        local hum = p.Character:FindFirstChild("Humanoid") local root = p.Character:FindFirstChild("HumanoidRootPart") or p.Character:FindFirstChild("Torso")
                                        if hum and root and hum.Health > 0 and root:IsA("BasePart") then
                                            local dist = (myRoot.Position - root.Position).Magnitude if dist < shortestDist then shortestDist = dist nearest = root end
                                        end
                                    end
                ...
                if currentTarget then
                    ...
                    if _G.Config.Prediction_Enabled and currentTarget:IsA("BasePart") then 
                        local vel = currentTarget.AssemblyLinearVelocity tPos = tPos + Vector3.new(vel.X, 0, vel.Z) * _G.Config.Prediction 
                    end
                    
                    local finalPos = tPos + Vector3.new(0, _G.Config.TP_Height, 0)
                    if finalPos.Y < 30 then finalPos = Vector3.new(finalPos.X, 30, finalPos.Z) end
                    
                    local hasTeleported = false
                    if lastTargetPositions[currentTarget] then
                        if (tPos - lastTargetPositions[currentTarget]).Magnitude > 100 then hasTeleported = true end
                    end
                    lastTargetPositions[currentTarget] = tPos

                    local targetPos = CFrame.new(finalPos, tPos)
                    local dist = (myRoot.Position - targetPos.Position).Magnitude
                    if dist <= 100 or hasTeleported then 
                        if currentTween then currentTween:Cancel() end 
                        myRoot.CFrame = targetPos 
                    else
                        local duration = dist / _G.Config.TP_Speed if duration < 0.05 then duration = 0.05 end 
                        currentTween = TweenService:Create(myRoot, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = targetPos}) currentTween:Play()
                    end
...
local dualRowNPC = Instance.new("Frame", tpNPCTab) dualRowNPC.Size = UDim2.new(1, -16, 0, 36) dualRowNPC.BackgroundTransparency = 1
local tpSelNPCBtn = Instance.new("TextButton", dualRowNPC) tpSelNPCBtn.Size = UDim2.new(0.48, 0, 1, 0) tpSelNPCBtn.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", tpSelNPCBtn).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(tpSelNPCBtn, _G.Config.TP_NPC) CreateBorder(tpSelNPCBtn) local tpSelNPCTxt = CreateButtonText(tpSelNPCBtn, "🎯 BẬT TP: OFF", Enum.Font.GothamBold, 11) ApplyButtonAnimation(tpSelNPCBtn) ToggleButtons["TP_NPC"] = {Btn = tpSelNPCBtn, Txt = tpSelNPCTxt, Name = "🎯 BẬT TP"}
tpSelNPCBtn.MouseButton1Click:Connect(function() _G.Config.TP_NPC = not _G.Config.TP_NPC ApplyToggleGradient(tpSelNPCBtn, _G.Config.TP_NPC) tpSelNPCTxt.Text = "🎯 BẬT TP: " .. (_G.Config.TP_NPC and "ON" or "OFF")
    if _G.Config.TP_NPC then _G.Config.TP_Player = false local b = ToggleButtons["TP_Player"] if b then b.Txt.Text = "🎯 BẬT TP: OFF" ApplyToggleGradient(b.Btn, false) end doMagnetLoop() else TempSkipNPC = {} if not _G.Config.TP_NPC then currentTarget = nil if currentTween then currentTween:Cancel() end end end end)
local skipNPCBtn = Instance.new("TextButton", dualRowNPC) skipNPCBtn.Size = UDim2.new(0.48, 0, 1, 0) skipNPCBtn.Position = UDim2.new(0.52, 0, 0, 0) skipNPCBtn.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", skipNPCBtn).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(skipNPCBtn, false) CreateBorder(skipNPCBtn) CreateButtonText(skipNPCBtn, "⏭️ SKIP NPC", Enum.Font.GothamBold, 11) ApplyButtonAnimation(skipNPCBtn)
skipNPCBtn.MouseButton1Click:Connect(function() if currentTarget and currentTarget.Parent then TempSkipNPC[currentTarget.Parent] = true currentTarget = nil if currentTween then currentTween:Cancel() end end end)
AddAdjust("TP NPC", "ĐỘ CAO (Y)", "TP_Height", 5) AddAdjust("TP NPC", "TỐC ĐỘ BAY", "TP_Speed", 50)
local SelectedNPCLabel = Instance.new("TextLabel", tpNPCTab) SelectedNPCLabel.Size = UDim2.new(1, -16, 0, 25) SelectedNPCLabel.BackgroundTransparency = 1 SelectedNPCLabel.Text = "Đang chọn: Tự động (Gần nhất)" SelectedNPCLabel.Font = Enum.Font.GothamBold SelectedNPCLabel.TextSize = 13 CreateTextGradient(SelectedNPCLabel)

local tpPlayerTab = ContentFrames["TP Player"].Frame
local dualRow = Instance.new("Frame", tpPlayerTab) dualRow.Size = UDim2.new(1, -16, 0, 36) dualRow.BackgroundTransparency = 1
local tpSelBtn = Instance.new("TextButton", dualRow) tpSelBtn.Size = UDim2.new(0.48, 0, 1, 0) tpSelBtn.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", tpSelBtn).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(tpSelBtn, _G.Config.TP_Player) CreateBorder(tpSelBtn) local tpSelTxt = CreateButtonText(tpSelBtn, "🎯 BẬT TP: OFF", Enum.Font.GothamBold, 11) ApplyButtonAnimation(tpSelBtn) ToggleButtons["TP_Player"] = {Btn = tpSelBtn, Txt = tpSelTxt, Name = "🎯 BẬT TP"}
tpSelBtn.MouseButton1Click:Connect(function() _G.Config.TP_Player = not _G.Config.TP_Player ApplyToggleGradient(tpSelBtn, _G.Config.TP_Player) tpSelTxt.Text = "🎯 BẬT TP: " .. (_G.Config.TP_Player and "ON" or "OFF")
    if _G.Config.TP_Player then _G.Config.TP_NPC = false local b = ToggleButtons["TP_NPC"] if b then b.Txt.Text = "🎯 BẬT TP: OFF" ApplyToggleGradient(b.Btn, false) end doMagnetLoop() else TempSkipPlayer = {} if not _G.Config.TP_Player then currentTarget = nil if currentTween then currentTween:Cancel() end end end end)
local skipBtn = Instance.new("TextButton", dualRow) skipBtn.Size = UDim2.new(0.48, 0, 1, 0) skipBtn.Position = UDim2.new(0.52, 0, 0, 0) skipBtn.BackgroundColor3 = Color3.new(1,1,1) Instance.new("UICorner", skipBtn).CornerRadius = UDim.new(0, 20) ApplyToggleGradient(skipBtn, false) CreateBorder(skipBtn) CreateButtonText(skipBtn, "⏭️ SKIP PLAYER", Enum.Font.GothamBold, 11) ApplyButtonAnimation(skipBtn)
skipBtn.MouseButton1Click:Connect(function() if currentTarget and currentTarget.Parent then local p = Players:GetPlayerFromCharacter(currentTarget.Parent) if p then TempSkipPlayer[p.Name] = true currentTarget = nil if currentTween then currentTween:Cancel() end end end end)
AddAdjust("TP Player", "ĐỘ CAO (Y)", "TP_Height", 5) AddAdjust("TP Player", "TỐC ĐỘ BAY", "TP_Speed", 50)
local SelectedPlayerLabel = Instance.new("TextLabel", tpPlayerTab) SelectedPlayerLabel.Size = UDim2.new(1, -16, 0, 25) SelectedPlayerLabel.BackgroundTransparency = 1 SelectedPlayerLabel.Text = "Đang chọn: Tự động (Gần nhất)" SelectedPlayerLabel.Font = Enum.Font.GothamBold SelectedPlayerLabel.TextSize = 13 CreateTextGradient(SelectedPlayerLabel)
            ...
            pcall(function()
                ...
                if tool then
                    ...
                    if remote then
                        ...
                        for i = 1, 3 do if remote:IsA("RemoteEvent") then remote:FireServer(direction, 1) elseif remote:IsA("RemoteFunction") then task.spawn(function() remote:InvokeServer() end) end end
                    end
                end
            end)
        end
...
local function CleanupESP(playerName) if ESP_Store[playerName]then pcall(function() if ESP_Store[playerName].BoxBill then ESP_Store[playerName].BoxBill:Destroy() end; if ESP_Store[playerName].TextBill then ESP_Store[playerName].TextBill:Destroy() end end) ESP_Store[playerName]=nil end local p = Players:FindFirstChild(playerName) if p and p.Character then if p.Character:FindFirstChild("ArisHL") then p.Character.ArisHL:Destroy() end local hrp = p.Character:FindFirstChild("HumanoidRootPart") if hrp then if hrp:FindFirstChild("ArisHitboxBox") then hrp.ArisHitboxBox:Destroy() end end end end
local function CleanupNPC(m) if NPC_Store[m]then pcall(function() if NPC_Store[m].Bill then NPC_Store[m].Bill:Destroy() end if NPC_Store[m].BoxBill then NPC_Store[m].BoxBill:Destroy() end end); NPC_Store[m]=nil end if m then if m:FindFirstChild("ArisHL_NPC") then m.ArisHL_NPC:Destroy() end local hrp = m:FindFirstChild("HumanoidRootPart") if hrp then if hrp:FindFirstChild("ArisHitboxBoxNPC") then hrp.ArisHitboxBoxNPC:Destroy() end end end end

local function GetHealthColor(percent) return Color3.new(1 - percent, percent, 0) end
...
RunService.Heartbeat:Connect(function(dt)
    if _G.Config.SafeMode and LocalPlayer.Character then
        ...
        if hum and hrp and hum.Health > 0 then
            ...
            if _G.IsFleeing then
                ...
                if not noclipConnection then toggleNoclip(true) end
                if hpPct < 0.55 then
                    local currentY = hrp.Position.Y if currentY < 90000 then hrp.CFrame = hrp.CFrame + Vector3.new(0, 100000 - currentY, 0) end
                else
                    _G.IsFleeing = false _G.IsReturning = true
                end
            elseif _G.IsReturning then
                if currentTween then currentTween:Cancel() end if not noclipConnection then toggleNoclip(true) end
                hrp.CFrame = hrp.CFrame - Vector3.new(0, 30000 * dt, 0)
                if hrp.Position.Y <= 150 then hrp.CFrame = CFrame.new(hrp.Position.X, 150, hrp.Position.Z) _G.IsReturning = false if noclipConnection and not isFarming then toggleNoclip(false) end end
            end
        end
    else _G.IsFleeing = false _G.IsReturning = false end
    ...
    if _G.IsFastDropping and LocalPlayer.Character then
        ...
        if hrp then
            ...
            if hrp.Position.Y <= 150 then hrp.CFrame = CFrame.new(hrp.Position.X, 150, hrp.Position.Z) _G.IsFastDropping = false if noclipConnection and not isFarming then toggleNoclip(false) end end
        end
    end

    local myRoot=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    local rgb = GetRGB() local shift = tick() * 0.15
    local seqOn = GetMovingColorSequence(Palettes.On, shift) local seqOff = GetMovingColorSequence(Palettes.Off, shift) local seqText = GetMovingColorSequence(Palettes.Text, shift * 1.5) local seqBorder = GetMovingColorSequence(Palettes.Border, shift * 1.8)
    for _, grad in ipairs(UIGradientList) do grad.Color = seqBorder end for _, grad in ipairs(TextGradientList) do if not grad:GetAttribute("CustomOnColor") then grad.Color = seqText end end for _, grad in ipairs(BtnGradientList) do if grad.Parent and grad.Parent:GetAttribute("IsOn") then grad.Color = seqOn else grad.Color = seqOff end end
    ...
    if _G.Config.Hitbox_P and _G.Config.LowHP_KS and myRoot then
        for _, p in Players:GetPlayers() do
            ...
            if char and hum and hrp and hum.Health > 0 then local dist = (hrp.Position - myRoot.Position).Magnitude if dist <= 2000 and (hum.Health / hum.MaxHealth) <= 0.3 then hasLowHPEnemy = true break end end
        end
    end
    ...
    for _,p in Players:GetPlayers()do
        ...
        if _G.Config.Hitbox_P then
            if _G.Config.LowHP_KS then if hasLowHPEnemy then if hp_percent <= 0.3 then hitboxShouldEnable = true end else if dist <= 2000 then hitboxShouldEnable = true end end else hitboxShouldEnable = true end
        end
        ...
        if hitboxShouldEnable then
            ...
            else hrp:SetAttribute("ArisHitboxActive", false) end
        else hrp:SetAttribute("ArisHitboxActive", false) end

        local hl = char:FindFirstChild("ArisHL")
        if _G.Config.ESP_Chams_P then if not hl then hl = Instance.new("Highlight", char); hl.Name = "ArisHL" end hl.FillColor = rgb; hl.Enabled = true else if hl then hl:Destroy() end end

        local hbBox = hrp:FindFirstChild("ArisHitboxBox")
        if hrp:GetAttribute("ArisHitboxActive") and _G.Config.Hitbox_Box then if not hbBox then hbBox = Instance.new("SelectionBox", hrp); hbBox.Name = "ArisHitboxBox"; hbBox.Adornee = hrp end hbBox.SurfaceColor3 = rgb else if hbBox then hbBox:Destroy() end end
        ...
        if _G.Config.ESP_Name_P or _G.Config.ESP_Health_P or _G.Config.ESP_Distance_P or _G.Config.ESP_Box_P or _G.Config.ESP_PVP then
            if not ESP_Store[p.Name] then
                ...
                local outF = Instance.new("Frame", boxBill); outF.Size = UDim2.new(1,0,1,0); outF.BackgroundTransparency = 1 Instance.new("UICorner", outF).CornerRadius = UDim.new(0, 16)
                local outS = Instance.new("UIStroke", outF); outS.Thickness = 2.5; outS.Color = Color3.new(1,1,1)
                local grad = Instance.new("UIGradient", outS) grad.Color = ColorSequence.new({ ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 230, 255)), ColorSequenceKeypoint.new(0.35, Color3.fromRGB(0, 0, 0)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 0, 0)), ColorSequenceKeypoint.new(0.65, Color3.fromRGB(0, 0, 0)), ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 50, 200)) })
                ...
                local txt = Instance.new("TextLabel", textB); txt.Size = UDim2.new(1,0,1,0); txt.BackgroundTransparency = 1; txt.Font = Enum.Font.GothamBold; txt.TextSize = 12 txt.TextStrokeTransparency = 0; txt.TextStrokeColor3 = Color3.new(0, 0, 0) txt.RichText = true 
            ...
            if _G.Config.ESP_PVP then
                if GetTrueStatus(p) then table.insert(espLines, "Status: <font color='#00A2FF'>PVP ON</font>") else table.insert(espLines, "Status: <font color='#A0A0A0'>PVP OFF</font>") end
            end
