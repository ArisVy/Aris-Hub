...
-- Kiểm tra có phải game Blox Fruits hay không
local PlaceId = game.PlaceId
local isBloxFruits = table.find({2753915549, 4442272183, 7449423635}, PlaceId) ~= nil

AddNotify({Title="ARIS HUB V1.1 - PATCHED",Description="Patched Infinite Stats & Universal Game Support",Duration=8})
...
_G.Config={
    ...
    SilentAim_NPC = false,
    SilentAim_Prediction = 0.127,
    SilentAim_WallCheck = false,
    
    -- Added Infinite Features (Mink v3, Energy, Soru)
    InfEnergy = false,
    InfSoru = false,
    InfObv = false
}
...
function InArea(POS)
    if not isBloxFruits then return {Name = ""} end
    local pos = typeof(POS) == "CFrame" and POS.Position or POS
    local wOrigin = workspace:FindFirstChild("_WorldOrigin")
    if wOrigin and wOrigin:FindFirstChild("Locations") then
        for _, v in next, wOrigin.Locations:GetChildren() do
...
function GetSpawnPoint(x)
    local wOrigin = workspace:FindFirstChild("_WorldOrigin")
    if not wOrigin or not wOrigin:FindFirstChild("PlayerSpawns") or not wOrigin.PlayerSpawns:FindFirstChild("Pirates") then return nil end
    for _, v in next, wOrigin.PlayerSpawns.Pirates:GetChildren() do
...
local World1 = isBloxFruits and workspace:FindFirstChild("_WorldOrigin") and workspace._WorldOrigin:FindFirstChild("Sea1") ~= nil
local World2 = isBloxFruits and workspace:FindFirstChild("_WorldOrigin") and workspace._WorldOrigin:FindFirstChild("Sea2") ~= nil
local World3 = isBloxFruits and workspace:FindFirstChild("_WorldOrigin") and workspace._WorldOrigin:FindFirstChild("Sea3") ~= nil
...
local SAPredictedPos = Vector3.zero
local FOVCircle = nil
...
function GetTrueStatus(target)
    ...
    local wOrigin = workspace:FindFirstChild("_WorldOrigin")
    local safeZones = wOrigin and wOrigin:FindFirstChild("SafeZones")
    if safeZones then
...
function IsBehindWall(targetHrp)
    local myRoot = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not myRoot then return false end
    local params = RaycastParams.new()
    params.FilterDescendantsInstances = {LocalPlayer.Character, targetHrp.Parent}
    params.FilterType = Enum.RaycastFilterType.Exclude
    local ray = workspace:Raycast(myRoot.Position, targetHrp.Position - myRoot.Position, params)
    return ray ~= nil
end
...
function GetClosestTargetForAim()
    ...
    function CheckTarget(targetRoot, targetPlayer)
        ...
        if _G.Config.SilentAim_WallCheck and IsBehindWall(targetRoot) then return end
...
local function IsHoldingFruit()
    local char = LocalPlayer.Character
    local tool = char and char:FindFirstChildOfClass("Tool")
    if tool and (tool.ToolTip == "Blox Fruit" or string.find(tool.Name:lower(), "fruit")) then
        return true
    end
    return false
end
...
if not getgenv().Hook_Initialized_Aris then
    ...
    OldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if _G.Config.SilentAim and SilentAimTarget and SAPredictedPos and not checkcaller() then
            local callingScript = getcallingscript() and tostring(getcallingscript()):lower() or ""
            if string.find(callingScript, "camera") or string.find(callingScript, "doublejump") or 
               string.find(callingScript, "skyjump") or string.find(callingScript, "airjump") or 
               string.find(callingScript, "dodge") or string.find(callingScript, "dash") or 
               string.find(callingScript, "geppo") or string.find(callingScript, "combatframework") then
                return OldNamecall(self, ...)
            end

            -- Removed LeftClickRemote / M1 AIMBOT hook due to user request!
            if method == "FireServer" and isBloxFruits then
                local rName = tostring(self)
                if rName == "RE/ShootGunEvent" then
                    if typeof(args[1]) == "Vector3" then args[1] = SAPredictedPos end
                    if type(args[2]) == "table" then args[2] = {SilentAimTarget} end
                    return OldNamecall(self, unpack(args))
                end
            end
            
            if (method == "Raycast" or string.find(method, "Ray")) and self == Workspace then
                local origin = (method == "Raycast") and args[1] or (args[1] and typeof(args[1]) == "Ray" and args[1].Origin)
                if origin and typeof(origin) == "Vector3" then
                    if method == "Raycast" then
                        args[2] = (SAPredictedPos - origin).Unit * 1000
                        return OldNamecall(self, unpack(args))
                    else
                        args[1] = Ray.new(origin, (SAPredictedPos - origin).Unit * 1000)
                        return OldNamecall(self, unpack(args))
                    end
                end
            end
    ...
    OldIndex = hookmetamethod(game, "__index", newcclosure(function(self, idx)
        if _G.Config.SilentAim and SilentAimTarget and SAPredictedPos and not checkcaller() and self == Mouse then
            local callingScript = getcallingscript() and tostring(getcallingscript()):lower() or ""
            if string.find(callingScript, "combatframework") or string.find(callingScript, "camera") or 
               string.find(callingScript, "playermodule") or string.find(callingScript, "popper") then
                return OldIndex(self, idx)
            end

            if idx == "Hit" or idx == "hit" then
                return CFrame.new(SAPredictedPos)
...
-- ==========================================
-- Added INFINITY ABILITY (Mink V3, Energy, Soru, Obv) logic here!
-- ==========================================
task.spawn(function()
    while task.wait(1) do
        if isBloxFruits and _G.Config.InfObv then
            pcall(function()
                local d = game.Players.LocalPlayer
                if d and d:FindFirstChild("VisionRadius") then
...
task.spawn(function()
    ...
    while task.wait(0.1) do
        if isBloxFruits and _G.Config.InfEnergy then
            pcall(function()
                local char = game.Players.LocalPlayer.Character
                if char and char:FindFirstChild("Energy") then
                    if char.Energy.Value < 100 then maxEnergy = char.Energy.Value end -- Update max if needed
                    char.Energy.Value = 99999
                end
            end)
        end
    end
...
task.spawn(function()
    while task.wait(0.1) do
        if isBloxFruits and _G.Config.InfSoru then
            pcall(function()
                local char = game.Players.LocalPlayer.Character
                if char and char:FindFirstChild("Soru") then
                    for _, R in next, getgc() do
                        if typeof(R) == "function" and getfenv(R).script == char.Soru then
                            for _, upv in next, getupvalues(R) do
...
SafeBtn.MouseButton1Click:Connect(function()
    ...
    AddNotify({Title="SAFE MODE",Description=_G.Config.SafeMode and "ON: Fly up infinitely when HP < 35%!" or "TURNED OFF!",Duration=3})
end)
...
AddGridToggle(nullGrid, "WALL CHECK", "SilentAim_WallCheck")
...
AddGridToggle(nullGrid, "INF MINK V3 (BLF)", "InfAblities") -- Mink v3 logic
AddGridToggle(nullGrid, "INF ENERGY (BLF)", "InfEnergy")
AddGridToggle(nullGrid, "INF SORU (BLF)", "InfSoru")
AddGridToggle(nullGrid, "INF OBSERVATION (BLF)", "InfObv")
...
AddAdjust("Combat", "SA PREDICT", "SilentAim_Prediction", 0.01, 0, 1)

-- [[ UI M1 DELAY ]] --
    _G.Config.FastM1_Delay = 0 
...
function doMagnetLoop()
    ...
    task.spawn(function()
        while _G.Config.TP_NPC or _G.Config.TP_Player do
            if _G.IsFleeing or _G.IsForcedDropping then
                task.wait(0.1)
                continue
            end
...
-- (Removed M1BF Kill Aura Block)

_G.FastV2 = true
local RS = game:GetService("ReplicatedStorage")
local P = Players.LocalPlayer

-- Safe check for modules to support non-Blox Fruits games
local N, C, WeaponData
local hit, atk
if isBloxFruits then
    pcall(function()
...
task.spawn(function()
    while task.wait() do
        if not _G.FastV2 or not _G.Config.FastM1 or not isBloxFruits then
            stopAnim()
            continue
        end
        if tick() - lastAttack2 < (_G.Config.FastM1_Delay or 0) then continue end
        ...
        pcall(function()
            ...
            local targets=getTargets(root,1000)
            if #targets>0 then
                atk:FireServer()
...
if isBloxFruits and enemies and characters then
...
RunService.Heartbeat:Connect(function()
    ...
    for _,target in pairs(enemies:GetChildren()) do
        ...
        if hrp and target:FindFirstChild("Humanoid") then
            pcall(function() remote:FireServer((hrp.Position - rootPos).Unit, 1) end)
        end
    end
...
local Remotes, Modules, Net, RE_ShootGunEvent, GunValidator, SUCCESS_SHOOT, SHOOT_FUNCTION
if isBloxFruits then
    pcall(function()
        Remotes = ReplicatedStorage:WaitForChild("Remotes")
        Modules = ReplicatedStorage:WaitForChild("Modules")
        Net = Modules:WaitForChild("Net")
        RE_ShootGunEvent = Net:WaitForChild("RE/ShootGunEvent")
        GunValidator = Remotes:WaitForChild("Validator2")
        SUCCESS_SHOOT, SHOOT_FUNCTION = pcall(function()
...
function GetClosestEnemy(range)
    if not isBloxFruits then return nil, nil end
    local char = Player.Character
    if not char then return end

    local pos = char:GetPivot().Position
    local enemiesFldr = workspace:FindFirstChild("Enemies")
    if not enemiesFldr then return end

    local closest, dist = nil, range
    local closestPart = nil

    for _,v in pairs(enemiesFldr:GetChildren()) do
        if IsAlive(v) then
...
function GetEnemiesInRange(range)
    ...
    for _, enemy in pairs(enemiesFldr:GetChildren()) do
...
RunService.Heartbeat:Connect(function()
    if not _G.Shoot or not isBloxFruits then return end
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.Config.WalkOnWater then
        pcall(function()
            local waterBase = game:GetService("Workspace"):FindFirstChild("Map") and game:GetService("Workspace").Map:FindFirstChild("WaterBase-Plane")
            if waterBase then
    ...
    if _G.Config.SilentAim then
        ...
        if SilentAimTarget then
            local velocity = SilentAimTarget.AssemblyLinearVelocity or Vector3.zero
            SAPredictedPos = SilentAimTarget.Position + (Vector3.new(velocity.X, 0, velocity.Z) * _G.Config.SilentAim_Prediction)

            local rgb = GetRGB()
            if FOVCircle then FOVCircle.Color = rgb end
            if TracerLine then
                local targetPos, targetOnScreen = Camera:WorldToViewportPoint(SAPredictedPos)
...
RunService.Heartbeat:Connect(function(dt)
    if _G.Config.SafeMode and LocalPlayer.Character then
        ...
        if hum and hrp and hum.Health > 0 then
            ...
            if hpPct <= 0.35 and not _G.IsFleeing then
                _G.IsFleeing = true
            end
            ...
            if _G.IsFleeing then
                ...
                hrp.CFrame = hrp.CFrame + Vector3.new(0, 300 * dt, 0)
    ...
    if myRoot then
        if _G.Config.AutoDrop and not _G.Config.SafeMode then
            if myRoot.Position.Y > 40000 and not _G.IsFleeing then
