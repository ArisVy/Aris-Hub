...
-- ==========================================
-- TỐI ƯU HÓA INFINITY ABILITY (Mink V3, Energy, Soru, Obv)
-- ==========================================
local SoruData = nil

local function FindSoruData(char)
    if not char or not char:FindFirstChild("Soru") then return end
...
game.Players.LocalPlayer.CharacterAdded:Connect(function(char)
    SoruData = nil
    task.delay(1.5, function() FindSoruData(char) end)
end)

task.spawn(function()
    while task.wait(0.1) do
        if not isBloxFruits then continue end
        
        local char = game.Players.LocalPlayer.Character
        if not char then continue end

        if _G.Config.InfEnergy and char:FindFirstChild("Energy") then
            char.Energy.Value = 99999
        end

        if _G.Config.InfObv then
        ...
        if _G.Config.InfSoru then
            if not SoruData then 
                FindSoruData(char) 
            end
            if SoruData then
                SoruData.LastUse = 0
...
AddGridToggle(nullGrid, "INF MINK V3 (BLF)", "InfAblities") 
...
task.spawn(function()
    if isBloxFruits then
        pcall(function()
            ...
            if Modules then
                N = require(Modules:WaitForChild("Net"))
                C = require(Modules:WaitForChild("CombatUtil"))
                WeaponData = require(Modules:WaitForChild("WeaponData"))
                hit = N:RemoteEvent("RegisterHit", true)
                atk = Modules.Net:WaitForChild("RE/RegisterAttack")
            end
        end)
    end
end)
...
-- ==========================================
-- LUỒNG FAST ATTACK (CHẠY ĐỘC LẬP & TỐC ĐỘ CAO)
-- ==========================================
local lastAttack2 = 0
...
task.spawn(function()
    while task.wait() do
        ...
        if tick() - lastAttack2 < (_G.Config.FastM1_Delay or 0) then 
            continue 
        end
        ...
        pcall(function()
            if not C or not WeaponData or not atk or not hit then return end
